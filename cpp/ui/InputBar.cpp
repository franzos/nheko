...
InputBar::emote(const QString &msg, bool rainbowify)
{
    ...
    room->timeline()->sendMessageEvent(emote, mtx::events::EventType::RoomMessage);
}
...
InputBar::notice(const QString &msg, bool rainbowify)
{
    ...
    room->timeline()->sendMessageEvent(notice, mtx::events::EventType::RoomMessage);
}
                ...
                const QString &blurhash)
{
    ...
    room->timeline()->sendMessageEvent(image, mtx::events::EventType::RoomMessage);
}
               ...
               uint64_t dsize)
{
    ...
    room->timeline()->sendMessageEvent(file, mtx::events::EventType::RoomMessage);
}
                ...
                uint64_t duration)
{
    ...
    room->timeline()->sendMessageEvent(audio, mtx::events::EventType::RoomMessage);
}
                ...
                const QString &blurhash)
{
    ...
    room->timeline()->sendMessageEvent(video, mtx::events::EventType::RoomMessage);
}
...
InputBar::sticker(CombinedImagePackModel *model, int row)
{
    ...
    room->timeline()->sendMessageEvent(sticker, mtx::events::EventType::Sticker);
}
...
MediaUpload::startUpload()
{
    if (!thumbnail_.isNull() && thumbnailUrl_.isEmpty()) {
        ...
        if (ba.size() >= (data.size() - data.size() / 10)) {
            nhlog::ui()->warn(
              "Thumbnail is not a lot smaller than original image");
            nhlog::ui()->debug(
