...
{
    return utils::replaceEmoji(info_.name.toHtmlEscaped());
}
...
RoomSettings::roomTopic() const
{
    return utils::replaceEmoji(
      utils::linkifyMessage(info_.topic
                              .toHtmlEscaped()
...
RoomSettings::changeName(QString name)
{
    ...
    if (newName == info_.name.toStdString()) {
        return;
    }
...
RoomSettings::changeTopic(QString topic)
{
    ...
    if (newTopic == info_.topic.toStdString()) {
        return;
    }
