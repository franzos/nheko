...
{
    try {
        info_ = Client::instance()->roomInfo(room_id_);
    } catch (const lmdb::error &) {
        nhlog::db()->warn("failed to retrieve room info from cache: {}", room_id_.toStdString());
    }

    try {
        auto members = Client::instance()->timeline(room_id_)->getMembers();
        addUsers(members);
...
MemberListBackend::fetchMore(const QModelIndex &)
{
    ...
    auto members = Client::instance()->timeline(room_id_)->getMembers();//cache::getMembers(room_id_.toStdString(), rowCount());
    addUsers(members);
    numUsersLoaded_ += members.size();
