...
AbstractButton {
    ...
    Image {
        ...
        source: blurhash ? ("image://blurhash/" + blurhash) : ("image://colorimage/:/images/image-failed.svg?" + GlobalObject.colors.buttonText)
        asynchronous: true
    ...
    MxcAnimatedImage {
        ...
        anchors.fill: parent
        roomm: room
        play: parent.hovered //!Settings.animateImagesOnHover || parent.hovered
        eventId: parent.eventId
    }

    onClicked : room.openImageOverlay(url, eventId, originalWidth, proportionalHeight) //Settings.openImageExternal ? room.openMedia(eventId) : TimelineManager.openImageOverlay(room, url, eventId, originalWidth, proportionalHeight);
